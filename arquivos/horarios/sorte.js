...
const { saveJSON, sendHours, identArroba, contarMin, alerandom, converterMin } = require(`../../definicoes.js`)

const sortepath = `./arquivos/horarios/sorte.json`
...
function sorteFunc(zerotwo, info, valor, text) {
    ...
    function sendMess() {zerotwo.sendMessage(from, {text: txt, mentions: menc}, {quoted: info})}
    ...
    if(AB < 0) {
        ...
        if((contarMin(sendHours("HH:mm")) + (Number(sendHours("DD")) !== Number(sorte[AB].tempo.dia) ? 1440 : 0)) >= (contarMin(sorte[AB].tempo.hora) + soma)) {
            ...
            if(sorte[AB].insi > 0) {
                ...
                zerotwo.sendMessage(from, {text: `Você já consultou sua sorte recentemente... E poderá fazer uma nova consulta em ${fim} 😊`}, {quoted: info})
            }
        }
    }
}
