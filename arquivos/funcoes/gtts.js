...
const GOOGLE_TTS_URL = 'https://translate.google.com/translate_tts'; // fix: http → https
const MAX_CHARS = 100;
const LANGUAGES = {
  'af': 'Afrikaans',
  'sq': 'Albanian',
...
function save(getArgs, filepath, text, callback) {
  ...
  async.eachSeries(text_parts, function(part, cb) {
    ...
    request({ uri: fullUrl, headers: headers, method: 'GET' }).pipe(writeStream);
...
function stream(getArgs, text) {
  ...
  return MultiStream(text_parts.map(function(part, index) {
    ...
    return request({ uri: fullUrl, headers: headers, method: 'GET' });
  }));
}
...
function tokenize(text) {
  ...
  var parts = text.split(new RegExp(pattern)).filter(p => p.length > 0);
  ...
  // fix: só faz substr se output não estiver vazio
  if (output.length > 0) output[0] = output[0].trimStart();
...
function createServer(getArgs, port) {
  ...
  var server = http.createServer(function(req, res) {
    ...
    var argsCallback = (queryData && queryData.lang && LANGUAGES[queryData.lang])
