...
module.exports = {
	...
	fromBuffer: async (buffer) => {
		...
		let ext;
		try {
			// Compatível com file-type v16 (CommonJS) e v17+ (ESM via fileTypeFromBuffer)
			if (typeof FileType.fromBuffer === 'function') {
				ext = await FileType.fromBuffer(buffer);
			} else if (typeof FileType.fileTypeFromBuffer === 'function') {
				ext = await FileType.fileTypeFromBuffer(buffer);
			} else {
				// file-type v18+ exporta como ESM, precisa de import dinâmico
				const ft = await import('file-type');
				const fn = ft.fileTypeFromBuffer || ft.fromBuffer;
				if (typeof fn === 'function') ext = await fn(buffer);
			}
		} catch(e) {}

		ext = ext || { mime: 'application/octet-stream', ext: 'bin' };
		return ext;
	},
	randomSelect(array) {
